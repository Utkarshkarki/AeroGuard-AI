...
High-reliability electronic components undergo rigorous Environmental Stress Screening (ESS), including Burn-In testing (operating components at elevated temperatures, e.g., $125^\circ\text{C}$, for up to 168 hours).
...
AeroGuard AI is a space-grade anomaly detection system that replaces rigid static cutoffs with Dynamic Lot Normalization (IQR Scaling), Predictive Time-Series Regression (0h to 24h forecasting 168h), SHAP Explainability, and a Deterministic Mission Assurance Rules Engine.
...
Python 3.12, Scikit-Learn, NumPy, Pandas, Joblib
...
Docker, Docker Compose (Air-Gapped Ready)
...
Inputs early telemetry points (, , and velocity ) to forecast  leakage.

Employs regularized Ridge Regression ( regularization, ) to eliminate overfitting on limited early time steps.

Achieves a Mean Absolute Error (MAE) of $2.16,\mu\text{A}$ on synthetic test datasets.
...
Module A: Robust Dynamic Limit (IQR Scaling)

Module B: Predictive Drift Regression
